...
void handleRXmenu(char c)
{
  ...
  if (CLI_menu == -1) {
    ...
    case 'S': {
      ...
      delay(200);
      if (RF_Mode == RECEIVED) {
        uint8_t rx_len = rfmGetPacketLength();
        rx_len = (rx_len > sizeof(tx_buf)) ? sizeof(tx_buf) : rx_len;
        rfmGetPacket(tx_buf, rx_len);
        if (rx_len > 0 && tx_buf[0] == 'U') {
          Serial.println(F("*****************************"));
